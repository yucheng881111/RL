	...
	size_t indexof(const std::vector<int>& patt, const board& b) const {
        ...
        for(int i = 0; i < patt.size(); ++i){
        	...
        	+------------------------+
 			...
 			|     4     2     8    16|
 			+------------------------+
 			board index 15 = b.at(15) = 16
 			board index  1 = b.at(1)  = 8
 			*/
...
public:
	...
	void update_episode(std::vector<state>& path, float alpha = 0.1) const {
		// TODO
		path.pop_back(); // pop (s2,s2,x,-1)

		float tmp = 0;
		// V(S'[i]) = V(S'[i]) + alpha * (r[i+1] + V(S'[i+1] - V(S'[i])))
		// path[i].value = path[i].value + alpha * (path[i+1].reward + path[i+1].value - path[i].value)
		for(int i = path.size() - 1; i >= 0; i--){
			// Line 716 => value = reward + estimate
			// => V(S'[i]) = value - reward

			// td_error = r[i+1] + V(S'[i+1]) - V(S'[i])
			// td_error =       tmp           - V(S'[i])
			float td_error = tmp - (path[i].value() - path[i].reward());

			// V(S'[i]) = V(S'[i]) + alpha * (r[i+1] + V(S'[i+1] - V(S'[i])))
			// => V(S'[i]) = update(V(S'[i]), alpha * td_error)
			tmp = path[i].reward() + update(path[i].after_state(), alpha * td_error);
			// previous layer (i+1) => store in a variable (tmp)
			// tmp = r[i+1] + V(S'[i+1])
		}
