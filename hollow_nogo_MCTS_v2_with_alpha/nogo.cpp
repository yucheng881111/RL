...
int main(int argc, const char* argv[]) {
	...
	for (int i = 1; i < argc; i++) {
		...
		} else if (para.find("--split=") == 0) {
			split = std::stoull(para.substr(para.find("=") + 1));
		...
		for (std::string command; std::getline(std::cin, command); ) {
			...
			if (args[0] == "play" || args[0] == "genmove") { // play a move, or generate a move and play
				...
				if (args[0] == "play") { // play a move
					...
					//debug << "play added state" << std::endl;
					steps++;
					...
					//debug << "genmove" << std::endl;
					moving_states.add_state(game.state());
					steps++;
					action::place move;

					//int best_move = alphago.get_move(moving_states);
					//move = who.take_action(game.state(), best_move);
					
					// mcts
					if (steps > split) {
						move = who.take_action(game.state(), -1);
					}
					// alphago
					else {
						int best_move = alphago.get_move(moving_states);
